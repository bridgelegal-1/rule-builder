...
import {Box, Stack, Typography, Paper, Divider, Select, MenuItem} from '@mui/material';

import './style.css';
import Context from './libs/context';
import operators from './libs/operators';
...
function reducer(state, action) {
  ...
  switch (action.type) {
    case 'add-group': {
      ...
      group.rules.splice(action.position, 0, emptyGroup());
      return query;
    }
    case 'add-rule': {
      const group = findNodeById(action.id, query);
      group.rules.splice(action.position, 0, emptyRule());
      return query;
    }
    case 'move-rule': {
    ...
    case 'remove-node': {
      function removeNote(id, q) {
        const parent = findParentById(id, q);

        if (parent?.id !== id) {
          parent.rules = parent.rules.filter((rule) => rule.id !== id);

          if (parent.rules.length < 1) {
            q = removeNote(parent.id, q);
          }
        }
        return q;
      }
      return removeNote(action.id, query);
    }
    case 'reset-query': {
      let { query } = action;
  ...
  (props) => {
    ...
    return state.id && context ? (
      <Context.Provider value={context}>
        <Box sx={{p: 4}}>
          <Paper variant='outlined' sx={{borderTop: 'none', position: 'relative'}}>
            <Divider textAlign='left' sx={{mt: '-18px', position: 'absolute', width: '100%'}}>
              <Stack direction='row' alignItems='center' spacing={1}>
                <Typography>If</Typography>
                  <Select
                    ...
                    sx={{height: 38}}
                    onChange={e => {
                      dispatch({ type: 'set-combinator', id: state.id, value: e.target.value });
                    }}
                  >
                    <MenuItem value='or'>Any</MenuItem>
                    <MenuItem value='and'>All</MenuItem>
                  </Select>
                <Typography>of the following conditions are met</Typography>
              </Stack>
            </Divider>
            <Box sx={{pt: 5, pb: 2, pr: 2}}>
